...
def generateSampleTime(S2Dir, out, nDays=5, startDate=False, lastDate=False):
    ...
    try:
        ...
        raise ImportError(
            'Datetime python library is needed to compute custom sample time')
    ...
    if lastDate is False or startDate is False:
        ...
        if S2 == []:
            ...
            S2 = [os.path.basename(os.path.dirname(S2Folder))
                  for S2Folder in S2]
        ...
        sampleTimeDT = [
            dt.datetime.strptime(
                date, '%Y%m%d') for date in sampleTime]
    ...
    #     Else, use lastDate found in folder
    # =============================================================================
    if lastDate is False:
        lastDate = sampleTimeDT[-1]
    else:
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        parser.add_argument(
            ...
            dest="s2dir",
            action="store",
            help="Sentinel-2 L2A Theia directory (if images not unzipped, will search for zip)",
            required=False)
        ...
        parser.add_argument(
            ...
            dest="nDays",
            action="store",
            help="New delta day to generate",
            required=True,
            type=int)
        ...
        parser.add_argument(
            ...
            dest="startDate",
            action="store",
            help="If specified, format (YYYYMMDD).",
            required=False,
            default=False)
