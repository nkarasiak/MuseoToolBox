...
def generateIndicesFromSITS(inSITS,outIndice,indice='ACORVI',sampleTime=False,comp=4,nbcore=1,ram=256):
    """
    ...
    try :
        ...
        raise ImportError("You need to have OTB available in python to use this script")
    ...
    bandOrder10 = np.array(['2','3','4','8','5','6','7','8A','11','12'])
    ...
    if comp == 4:
        ...
        raise Exception('This script only works with 4 or 10 composant per date')
    ...
    for i in range(1,nDates+1):
        ...
        if not glob.os.path.exists(indiceFolder):
            glob.os.makedirs(indiceFolder)
        if sampleTime:
            currentIndice = indiceFolder+'/'+indice+'_'+str(TIMESAMPLE[i-1])+'.tif'    
        else:
        ...
        for band in set(bandsToChange):
            ...
            indiceBandMathExp = indiceBandMathExp.replace(band,'im1b'+str(int(newBand)))
    ...
    indiceFiles = sorted(glob.glob(glob.os.path.join(indiceFolder,indice,'*.tif')))
    indiceFilesToBash = listToStr(indiceFiles)
    buildVrt = 'gdalbuildvrt -separate {} {}'.format(outIndice,indiceFilesToBash)
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        print("or : ", prog, " -h")
        print("example 1 : python %s -inSITS /tmp/SITS.tif -outIndice /tmp/indice/ACORVI.vrt -indice ACORVI"%sys.argv[0])
        ...
        parser.add_argument("-outIndice","--out", dest="outIndice", action="store", \
        help="Output name (vrt) of the indice (each date will be saved as a tif in subfolder)", \
        required = True, type = str)
        
        parser.add_argument("-indice","--indice", dest="indice", action="store",\
        help="Indice to generate",choices=list(indicesDict.keys()),default='ACORVI')

        parser.add_argument("-sampleTime","--s", dest="sampleTime", action="store", \
        ...
        generateIndicesFromSITS(inSITS=args.inSITS,outIndice=args.outIndice,indice=args.indice,\
                                sampleTime=args.sampleTime,comp=args.comp,nbcore=args.nbcore,ram=args.ram)
