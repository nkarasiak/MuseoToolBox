...
def computeSITS(
        S2Dir,
        outSITS,
        resample20mBands=False,
        resampleCSV=False,
        unzip=False,
        cloudMask=None,
        OTBPythonBinding=True,
        checkOutliers=True,
        nbcore=1,
        ram=256):
    """
    ...
    if OTBPythonBinding is True:
        ...
        if resample20mBands:
            print(
                "WARNING : Resample 20m bands at 10m is not available without OTBPythonBinding")
            sys.exit(1)
    ...
    if unzip:
        ...
        if resample20mBands:
            formula = "parallel -j " + \
                str(nbcore) + " unzip -n {} *FRE_B?.tif *FRE_B??.tif *CLM_R1* *.xml *.jpg -d " + S2Dir + " ::: " + os.path.join(S2Dir, "*.zip")
        else:
            formula = "parallel -j " + \
                str(nbcore) + " unzip -n {} *FRE_B2*.tif *FRE_B3*.tif *FRE_B4*.tif *FRE_B8*.tif *CLM_R1* *.xml *.jpg -d " + S2Dir + " ::: " + os.path.join(S2Dir, "*.zip")
    ...
    if OTBPythonBinding:
        ...
        os.system(
            'gdalbuildvrt -separate {0}{1}'.format(CLOUDS_temp, listToStr(cloudsToMask)))
    ...
    for i in orderedSITS:
        for j in band10m:
            fourBandsToVrt.append(
                glob.glob(
                    os.path.join(
                        i, '*FRE_B{}.tif'.format(j)))[0])
        if resample20mBands:
            for k in band20m:
                sixBandsToVrt.append(
                    glob.glob(
                        os.path.join(
                            i, '*FRE_B{}.tif'.format(k)))[0])
    ...
    if OTBPythonBinding:
        ...
        os.system(
            'gdalbuildvrt -separate {0}{1}'.format(SITS_temp, listToStr(fourBandsToVrt)))
...
if __name__ == "__main__":
    if len(sys.argv) == 1:
        ...
        computeSITS(
            S2Dir=args.s2dir,
            outSITS=args.outSITS,
            resample20mBands=args.resample20mBands,
            resampleCSV=args.resampleCSV,
            unzip=args.unzip,
            cloudMask=args.cloudMask,
            OTBPythonBinding=args.OTBPythonBinding,
