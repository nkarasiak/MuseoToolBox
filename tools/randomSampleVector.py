...
class samplingMethods:    
    ...
    def random(split=0.5,maxSamplesPerClass=None,seed=None,nIter=5):
        """
        ...
        """
        samplingType = 'random'
        return [samplingType,dict(split=split,maxSamplesPerClass=maxSamplesPerClass,seed=seed,nIter=nIter)]
...
class sampleSelection:
    def __init__(self,samplingMethod):
        ...
        if self.samplingType == 'random':
            ...
            maxSamplesPerClass = samplingMethod[1]['maxSamplesPerClass']
            if maxSamplesPerClass:
                def randomSamplePerClass(Cpos,nSamp):
                    train = np.random.permutation(Cpos)[:nSamp]
                ...
                def randomPerClass(FIDs,maxSamples,seed):
                    ...
                    for C in np.unique(FIDs):
                        ...
                        np.random.seed(seed)
                        trvl = randomSamplePerClass(Cpos,maxSamples)
                        train = np.concatenate((train,trvl[0]))
                        valid = np.concatenate((valid,trvl[1]))
                    return train,valid
                
                self.crossvalidation = [randomPerClass(FIDs,maxSamplesPerClass,seed+i) for i in range(nIter)]
            else:
...
if __name__ == '__main__':
    ...
    sampling = samplingMethods.random(split=0.5,nIter=5)
