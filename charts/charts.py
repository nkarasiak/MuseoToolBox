...
def plot_comOrOm(cm,title='Omission',ylabels=False,xlabels=False,vmin=False,vmax=False,xlabel=False,ylabel=False,\
                 thresold=49,alpha=False,legend=False,percent=True,xhorizontalalignment=False,cmap=plt.cm.Oranges,lastCorner=False,cm2=False,cmap2=plt.cm.Greens,xlabelsPos="bottom",diag=False,\
                 suptitle=False,xrotation=False,verticalalignment='baseline',thresold2=49,vmin2=False,vmax2=False,dpi=False,toround=0):
    if not alpha:
        alpha = 1.0
    ...
    if cm2 is not False:
        ...
        pl1 = ax.imshow(cm,interpolation='nearest',cmap=cmap,vmin=vmin,vmax=vmax,alpha=alpha)
        
        if not vmin2:
            vmin2 = np.amin(cm2)
            print('vmin2 not detected')
        if not vmax2:
            vmax2 = np.amax(cm2)
        pl2 = ax.imshow(cm2,interpolation='nearest',cmap=cmap2,vmin=vmin2,vmax=vmax2,alpha=alpha)
        
    
    else:
        fig = plt.subplots(figsize=(14,7),dpi=dpi)
        fig = plt.imshow(cm, interpolation='nearest', cmap=cmap,vmin=vmin, vmax=vmax,alpha=alpha)
    ...
    if ylabels:
        ax.set_yticklabels(ylabels,rotation=0,fontsize=10,horizontalalignment='right')
    ...
    if xlabels:
        ...
        ax.set_xticklabels(xlabels,rotation=rotation,fontsize=10,horizontalalignment=xhorizontalalignment)
    else:
        ...
        if not np.isnan(cm[i,j]):
            ...
            if not np.ma.is_masked(cm[i,j]):
                ...
                if lastCorner and i==cm.shape[0]-1 and j==cm.shape[1]-1:
                    plt.text(j,i,'',va=verticalalignment)
                else:
                    plt.text(j, i, numberToPlot,
                     horizontalalignment="center",
                     color="white" if cm[i, j] > thresold else "#444444",va=verticalalignment)
            if cm2 is not False:
                if not np.ma.is_masked(cm2[i,j]):
                    ...
                    if lastCorner and i==cm.shape[0]-1 and j==cm.shape[1]-1:
                        ...
                        plt.text(j, i, numberToPlot,
                         horizontalalignment="center",
                         color="white" if cm2[i,j] > thresold2 else "#444444",va=verticalalignment)
    ...
    if not xlabels:
        plt.xticks(np.arange(cm.shape[1]),tick_marks, rotation=45)
    plt.tight_layout()
...
def plotConfusionMatrix(cm,title='',ylabels=False,xlabels=False,vmin=False,vmax=False,xlabel=False,ylabel=False,\
    ...
    if subplot:
        try:
            ...
            raise ImportError('Can\'t import gridspec from matplotlib')
